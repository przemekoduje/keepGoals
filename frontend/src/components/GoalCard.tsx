...
export const GoalCard: React.FC<GoalCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative flex flex-col rounded-[24px] p-5 transition-shadow duration-300 border outline-none ${onClick ? "cursor-pointer" : ""} ${
        goal.is_completed
          ? "bg-slate-50/50 dark:bg-slate-900/30 border-slate-200 dark:border-slate-800/60 opacity-85 hover:opacity-100"
          : "bg-white dark:bg-slate-800/90 border-slate-300/60 dark:border-slate-700/60 shadow-sm hover:shadow-md"
      }`}
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3 gap-2">
        <div className="flex flex-wrap items-center gap-1.5 mt-1">
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border ${horizonInfo.classes}`}
          >
            {horizonInfo.icon}
            {horizonInfo.label}
          </span>
        ...
        <div className="flex items-center space-x-0.5">
          <button
            ...
            className={`p-1.5 rounded-full transition-colors ${
              goal.is_completed
                ...
                : "text-slate-300 hover:text-emerald-600 hover:bg-slate-50 dark:hover:bg-slate-700/50"
            }`}
          >
            {goal.is_completed ? (
          ...
          <div className="relative">
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-full hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
            >
      ...
      {/* Goal Title */}
      <h3
        className={`text-[17px] font-bold mb-1.5 leading-tight ${
          goal.is_completed
      ...
      {/* Goal Description */}
      {goal.description && (
        <div className="text-[13px] text-slate-600 dark:text-slate-300 mb-4 prose prose-sm dark:prose-invert max-w-none line-clamp-2">
      ...
      <div className="mt-auto pt-3">
        ...
        <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700/70 rounded-full overflow-hidden">
          <div
        ...
        {goal.key_results && goal.key_results.length > 0 && (
          <div className="mt-3 border-t border-slate-100 dark:border-slate-700/60 pt-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setExpandedKr(!expandedKr);
              }}
              className="flex items-center justify-between w-full text-[11px] font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors py-1"
            >
              ...
              >
                {goal.key_results.map((kr, idx) => {
                  ...
                  return (
                    <div
                      key={kr.id || idx}
                      className="bg-slate-50/50 dark:bg-slate-900/20 rounded-lg p-2 text-xs"
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-slate-700 dark:text-slate-300 truncate pr-2">
                          {kr.title}
                        </span>
                        <span className="text-slate-500 dark:text-slate-400 font-mono text-[10px] whitespace-nowrap">
                        ...
                        {onUpdateKeyResult && !goal.is_completed && (
                          <div className="flex items-center space-x-0.5">
                            <button
                              ...
                              className="p-1 rounded text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 transition-colors"
                              title="Zmniejsz"
                            >
